...
public class AuthController {
    ...
    public AuthController(IAuthService authService, IRegisterService registerService) {
     ...
     public ResponseEntity<ApiResponse<UserResponseDTO>> register(@Valid @RequestBody RegisterUserRequest request){
        ...
        System.out.println(RoleType.RIDER.toString());
        if (request.getRole()== null || !(request.getRole().equals(RoleType.RIDER.toString()) || request.getRole().equals(RoleType.DRIVER.toString())))
        {
            System.out.println(request.getRole().equals(RoleType.RIDER.toString()));
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(ApiResponse.error("Role must be RIDER or DRIVER.",null));
        }
        UserResponseDTO user = registerService.register(request);
     return ResponseEntity.status(HttpStatus.CREATED).body(ApiResponse.<UserResponseDTO>success("User Registered", user));
